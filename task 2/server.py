...
if __name__ == '__main__':
    port = 5698
    host_ip_client_addresses = socket.gethostbyname(socket.gethostname())   # Getting the current device's IP client_addressess
    server_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)  # Initiating a TCP server socket

    server_socket.bind((host_ip_client_addresses, port))  #Bindong the socket with the IP and port Number
    server_socket.listen(10)  # Maximum number of connections = 10
    print(f"Server running on  http://{host_ip_client_addresses}:{port}\n")
